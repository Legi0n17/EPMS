...
lblinfo=Label(Tops,font=('arial',45,'bold'),text="Employee Payment Management System ",bd=10,fg="green")#heading 
lblinfo.grid(row=0,column=0)# grid to put in tabular 
...
def reset():
  ...
  Employer.set("")
  NINumber.set("")
  txtpayslip.delete("1.0",END)#1st ch to last
  
#Payslip
def enterinfo():
  sqlconnect = sqlite3.connect("C:\\Users\\Roushan Prakash\\OneDrive\\Desktop\\Project 7th Sem\\Employee.db")
  ...
  txtpayslip.insert(END,"Gross Payable :\t\t"+Payable.get()+"\n\n")
  #databse_connect
  name = Name.get()
  address = Address.get()
  empid = Employer.get()
  ...
  wagehour = wageshour.get()
  tax = Taxable.get()
  gross = Payable.get()
  query1 = "INSERT INTO Details VALUES('{eid}','{nm}','{mob}','{add}','{hrsworked}','{wghr}','{tax}','{gs}','{net}')".format(eid = empid, nm = name, mob = mobile, add = address, hrsworked = hoursworked, wghr = wagehour, tax = tax, gs = gross, net = netpay)
...
DateOfOrder.set(time.strftime("%d/%m/%Y"))#day of mnth/month/year incl. century 
...
lblEmployer=Label(fla,text="Employer ID",font=('arial',16,'bold'),bd=20,fg="red",bg="powder blue").grid(row=1,column=0)
lblNINumber=Label(fla,text="Mobile",font=('arial',16,'bold'),bd=20,fg="red",bg="powder blue").grid(row=1,column=2)
...
lblGrossPay=Label(fla,text="Gross Pay",font=('arial',16,'bold'),bd=20,fg="red",bg="powder blue").grid(row=4,column=0)
